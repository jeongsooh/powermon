...
def calculate_price_perload(peakload, highload, lowload, userid):
  total_amount = 0
  now = datetime.now()
  ...
  # price = price_table[HV][USAGE][Type1][time_range][season]
  p_price = price_table[HV][USAGE][Type1][PLOAD][season]
  h_price = price_table[HV][USAGE][Type1][HLOAD][season]
  l_price = price_table[HV][USAGE][Type1][LLOAD][season]

  total_usage = p_price * peakload + h_price * highload + l_price * lowload

  queryset_user = User.objects.filter(userid=userid).values()
  user_id = queryset_user[0]['id']
  queryset = Variables.objects.filter(user_id=user_id).values()
  base_fee = price_table[HV][BASIC] * now.day / 30 * queryset[0]['baselined'] / 1000
